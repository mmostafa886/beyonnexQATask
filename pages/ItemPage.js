...
class ItemsPage {
  ...
  async selectCheapestItem(itemType) {
    const letterCaseIgnore = new RegExp(itemType, "i");
    const materialContainingItems = Selector(".text-center.col-4")
      .child(1)
      .withText(letterCaseIgnore);
    /**This will retrieve a list of item names where the provided (itemType)/material is part of the item name (despite the letters case as it was noticed that
     * some of the item names are written in capital & others are in small letters)
    ...
    for (let i = 0; i < itemsCount; i++) {
       ...
       */
      if (typeof leastPrice === "undefined" || price < leastPrice) {
        leastPrice = price;
        selectedItem = item;
